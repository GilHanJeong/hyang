...
각 2172×724. PNG alpha 확인. 원경은 전경의 1/3 속도. 세 마을을 반복하고 전체 주기로 스크롤 좌표를 재배치합니다. 이미지 높이 전체를 100svh에 맞추며 가로 비율은 3:1로 유지합니다. 바닥은 1번 이미지 하단 9%를 공통 레이어로 재사용해 패널별 두께 차이를 가립니다. 방향 버튼, 방향키, 가로 트랙패드, 모바일 스와이프 지원. 세로 휠은 강제 변환하지 않습니다. 게시물은 아직 연결하지 않았습니다.
